...
Connect this repository to Vercel and expose it on `adr.moretes.com`.

## 1. Vercel project

https://vercel.com → **Add New… → Project** → import
`fernandofatech/adr-decision-platform`. Framework: Next.js. Root directory:
...
Vercel: **Settings → Domains → Add** → `adr.moretes.com`.

## 5. Cloudflare DNS

Add CNAME `adr` → `cname.vercel-dns.com`, **DNS only** (gray cloud).

## 6. Verify

```bash
curl -I https://adr.moretes.com
```
